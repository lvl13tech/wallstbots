...
SECTORS = {
    ...
    "CAT":"INDUSTRIALS","RTX":"INDUSTRIALS","HON":"INDUSTRIALS","GE":"INDUSTRIALS","SPCX":"INDUSTRIALS",
...
def enrich_position(pos, prices, prev_closes):
    ...
    return mark_position(pos, prices, prev_closes, crypto=False)
...
def run_oracle_decision(prices, prev_closes, hist_data, starting_capital, week_str):
    ...
    # Projected weekly return = weighted avg 5d momentum of picks
...
def run_wizard_decision(prices, prev_closes, hist_data, starting_capital, month_str):
    ...
    # Projected monthly return = weighted avg 20d momentum of picks
...
def trigger_portfolio_snapshots(secrets):
    ...
    per-portfolio daily performance snapshots for all active wallstbots portfolios.
    """
    if _requests is None:
        return
    api_url      = secrets.get("api_url") or os.environ.get("TRACKER_API_URL", BACKEND_URL)
...
def main():
    ...
    if raw is None:
        ...
        try:
            api_url = secrets.get("api_url") or os.environ.get("TRACKER_API_URL", BACKEND_URL)
            _fb = _requests.get(
                f"{api_url}/public/tracker/state?platform=wallstbots", timeout=15
            )
    ...
    if not funds:
        print("[wallstbots] run ABANDONED -- no fund data available from disk OR the live API. "
              "Refusing to publish an empty site. Run full_reset_all.py --platform wallstbots "
    ...
    day_ref_closes = build_day_reference(state_data, snapshots, today_iso, prev_closes, "wallstbots")
    ...
    b13_banked = (
        ...
        if (prev_b13_strategy or {}).get("day") == today_iso else 0.0
    )
    ...
    drawdown_hit = check_drawdown(EQUITY_CFG, b13_day_open, stored_positions, prices, b13_banked)
    ...
    if b13_inception >= today_iso:  # do not trade on inception day itself
        ...
        # 3:30 close-out. The old behavior flattened the ENTIRE book and re-picked
        # a fresh one -- that is the churn that bled aistocks -3.63% on 2026-07-20
        ...
        # Accounting decision is HOLD (positions now flat), but for DISPLAY we
        # preserve today's real trade story: morning decision (TRADE), the real edge
        # score that triggered it, and the real rationale. No "HOLD -- close-out"
        # message (that wrongly reads as "didn't trade").
        b13_decision  = "HOLD"
        b13_positions = []
        b13_picks     = (prev_b13_strategy or {}).get("picks", [])
        ...
        _fresh = run_bot13_equity(
            ...
            b13_day_open + b13_banked, today_iso, prev_b13_strategy,
        ...
        _stop_pct     = float(EQUITY_CFG.get("stop_internal", 1.35))
        LOSS_EXIT_PCT = 1.0
        ...
        elif _stored_pos and (prev_b13_strategy or {}).get("day") != today_iso:
    ...
    for fid in FUND_ORDER:
        ...
        if fid == "bot13":
            ...
            b13_proj, _b13_samps, _b13_lastset = resolve_edge_score(prev_b13_strategy, b13_proj, b13_picks, today_iso, not window_open)  # FREEZE decision-time edge score for the day (never becomes Today's Change)
    ...
    if reset_occurred_mid_run("wallstbots", _loaded_inceptions, _rg_api):
        print("[wallstbots] run ABANDONED (reset-collision guard) -- nothing written.")
        sys.exit(0)
