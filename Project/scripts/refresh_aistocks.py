...
def main():
    ...
    if not funds:
        print("[aistocks] run ABANDONED -- no fund data available from disk OR the live API. "
              "Refusing to publish an empty site. Run full_reset_all.py --platform aistocks "
    ...
    day_ref_closes = build_day_reference(state_data, snapshots, today_iso, prev_closes, "aistocks")
    ...
    if b13_inception >= today_iso:  # do not trade on inception day itself
        ...
        # (23 sells, 6 winners) and it would have dumped CBRS (+$1,204 six-hour
        # hold) along with the loser. No re-picking, no revenge re-entries.
