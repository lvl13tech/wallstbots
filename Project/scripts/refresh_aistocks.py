    ...
    if _boundary.get("date") == _prev_snap_date and isinstance(_boundary.get("prices"), dict) and _boundary.get("prices"):
        ...
        for _s, _v in _boundary["prices"].items():
            try:
                ...
                pass
        print(f"[aistocks] day reference = boundary prices from {_boundary['date']} ({len(_boundary['prices'])} assets)")
    else:
        day_ref_closes = prev_closes
        print("[aistocks] day reference = feed prev_closes (no stored boundary for yesterday yet)")
