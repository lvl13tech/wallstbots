...
const REPORT_PLATFORM = 'bitbot13';   // this site's platform (per-site branding value)
const REPORT_BRAND    = 'BitBot13';
...
function renderHome() {
  ...
  $('app').innerHTML =
    ...
    + '<img src="assets/logo-aistocks.png" alt="aistocks.tech" style="width:100%;max-width:320px;height:auto;display:block;margin:0 auto 14px;border-radius:8px"></a>'
    + '<p style="color:var(--muted);font-size:13px;line-height:1.6;margin:0 0 14px;flex:1">The same AI intelligence applied to 50 hand-picked AI &amp; Quantum stocks. Daily signals, live leaderboards, and downloadable monthly statements.</p>'
...
function renderHowItWorks() {
  ...
  $('app').innerHTML = '<h1>How It Works</h1>'
    ...
    + '<p style="font-size:15px;line-height:1.7;margin:0 0 10px">Most trading advice is noise — screenshots, hot takes, and “trust me.” We built something you can actually watch instead: five strategies, each handed the same money and the same coins, trading head-to-head every single day. Three are active bots — daily, weekly, monthly. Two are passive benchmarks that show what plain buy-and-hold would have done.</p>'
    ...
    + '<p>Three active bots. Two passive benchmarks. The same money, the same market. The best strategy wins in the open — every day. Crypto never sleeps, and neither do the bots.</p></div></div>'
    + getYoursHint('Point these five bots at your own coins — start free.');
}
...
function renderGetYours() {
  ...
  $('app').innerHTML =
    ...
    + '<p style="color:var(--muted);font-size:15px">Pick up to 50 coins from any chain. Daily, weekly, and monthly bots. Crypto news filtered to your picks. Monthly statements you can download.</p>'
...
async function gyFreeSignup() {
  ...
  try {
    const r = await fetch('https://wallstbots-backend-868128114349.us-east1.run.app/auth/signup-free', {
      ...
      body: JSON.stringify({ email, password, platform: 'bitbot13', turnstile_token: (window.turnstile ? turnstile.getResponse() : '') })
    });
...
async function startStripeCheckout() {
  ...
  try {
    const r = await fetch(API_BASE + '/stripe/create-checkout', {
      ...
      body: JSON.stringify({ tier: GY_TIER, cycle: GY_CYCLE, platform: 'bitbot13', ref_code: GY_VALID ? GY_REF : '' }),
    });
    const d = await r.json();
